...
bool Modele::estMemeCours(Cours c, Cours c2){
	if (c.getNomCours().compare(c2.getNomCours())==0){
		return true;
	}
	else{
		return false;
...
vector<Cours> Modele::getListeCoursProf(Professeur e){
	//cout << "L'etudiant " << e.getPrenom() << " est inscrit(e) au(x) cour(s) suivant : \n" << endl;
	vector<Cours> *listeCoursProf = new vector<Cours>();
	for (unsigned i = 0; i < listeCours.size(); i++){
				listeCoursProf->push_back(getCoursAt(i));	
			}
	return *listeCoursProf;
}
...
vector<Cours>* Modele::getCoursEtuPasInscrit(Etudiant e){
	vector<Cours> tmp = getListeCoursEtudiant(e);
	vector<Cours> *listeCoursPasInscrit = new vector<Cours>();
	for (unsigned i = 0; i < listeCours.size(); i++){	
		if (!existeCours(getCoursAt(i), tmp)){			
					listeCoursPasInscrit->push_back(getCoursAt(i));
			}
	}	
	return listeCoursPasInscrit;
}
...
bool Modele::existeCours(Cours c, vector<Cours> v){
	...
	for (unsigned i = 0; i < v.size(); i++){
		if (v.at(i).getNomCours()== c.getNomCours()){
			return true;
		}
	}
	return false;
