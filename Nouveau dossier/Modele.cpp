...
vector<Cours> Modele::getCoursEtuPasInscrit(Etudiant e, Cours c){
	vector<Cours> *listeCoursPasInscrit = new vector<Cours>();
	for (unsigned i = 0; i < listeCours.size(); i++){
		for (unsigned j = 0; j < getListeCoursEtudiant(e).size(); j++){
			if (!estMemeCours(getCoursAt(i), c)){
				if (!existeCours(getCoursAt(i), *listeCoursPasInscrit)){
